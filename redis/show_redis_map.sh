#!/bin/bash
# Writed by jindongli on 2019/2/15
#
# Redis集群部署注意事项：
...
do
    ...
    tag_save=
    rdb_stat=
    tag_save=`redis-cli -c -h $master_node_ip -p $master_node_port config get save |sed -n 2p`

    if test "$tag_save" = "" ; then 
            rdb_stat=false
    else
            rdb_stat=true
    fi

    printf "[%02d][MASTER]  %-20s \033[0;32;31m%s\033[m%s \033[m%s\n" $index "$master_node" "$master_id" "$tag" "$rdb_stat"
...
do
    ...
    tag_save=
    rdb_stat=
    tag_save=`redis-cli -c -h $slave_node_ip -p $slave_node_port config get save |sed -n 2p`
    ...
    n=$(($index % 2))
    if test $n -eq 0; then
        printf "[%02d][SLAVE=>MASTER] \033[1;33m%21s\033[m  =>  \033[1;33m%s\033[m%s \033[m%s\n" $index $slave_node $master_node "$tag" "$rdb_stat"
    else
        printf "[%02d][SLAVE=>MASTER] %21s  =>  %s%s %s \n" $index $slave_node $master_node "$tag" "$rdb_stat"
    fi
